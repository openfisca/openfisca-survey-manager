...
class Calibration:
    ...
    def __init__(
        ...
        if len(entities) == 0:
            ...
            entity_id_variable_link = variable_instance_by_variable_name[
                parameters["id_variable_link"]
            ].entity.key
            assert entity_id_variable in entities, (
    ...
    def _build_calmar_data(self) -> dict:
        ...
        data[self.target_entity][self._initial_weight_name] = (
            self.initial_weight * self.filter_by
        )
        period = self.period
        ...
        if len(self.entities) == 2:
            for entity in self.entities:
                if entity == self.target_entity:
                    data[entity]["id_variable"] = (
                        self.simulation.adaptative_calculate_variable(
                            self.parameters["id_variable"], period=period
                        )
                    )
                else:
                    data[entity][self.simulation.weight_variable_by_entity[entity]] = (
                        self.initial_weight_by_entity[entity]
                    )
                    data[entity]["id_variable"] = (
                        self.simulation.adaptative_calculate_variable(
    ...
    def calibrate(self, inplace=False):
        ...
        if self.other_entity_count is not None:
            simple_margins_by_variable["total_population_smaller_entity"] = (
                self.other_entity_count
            )
    ...
    def summary(self):
        ...
        margins_df.loc["entity_count", "initial"] = (
            self.initial_weight * self.filter_by
        ).sum()
