...
def create_input_dataframe(entities=1):
    ...
    if entities == 2:
        ...
        df = {
            "main_entity": df1,
            "target_entity_name": "main_entity",
            "second_entity": df2,
        }
    return df
...
def test_calmar():
    ...
    data[data["target_entity_name"]]["weight_ratio"] = (
        pondfin_out / data[data["target_entity_name"]].POND
    )
    weight_ratio = (
...
def test_calmar_2_entities():
    ...
    pondfin_ind = data["main_entity"].merge(data["second_entity"], on="id_variable")[
        "final_pond"
    ]
