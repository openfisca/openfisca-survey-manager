...
@pytest.mark.order(
    after="test_write_parquet.py::TestWriteParquet::test_write_parquet_one_file_per_entity"
)
class TestParquet(TestCase):
    ...
    def test_load_single_parquet_monolithic(self):
        ...
        sim_res = (
            simulation.calculate("housing_tax", period.this_year).flatten().tolist()
        )
    ...
    def test_load_parquet_batch(self):
        ...
        assert (
            df.columns
            == ["household_id", "rent", "household_weight", "accommodation_size"]
        ).all()
        ...
        while True:
            try:
                ...
                simulation = survey_scenario.simulations["baseline"]
                sim_res = (
                    simulation.calculate("housing_tax", period.this_year)
                    .flatten()
                    .tolist()
                )
                results["housing_tax"] += sim_res
        ...
        assert math.isclose(sum(results["income_tax"]), sum([195.00001525878906, 3.0, 510.0000305175781, 600.0, 750.0]))
