...
def randomly_init_variable(tax_benefit_system, input_dataframe_by_entity, variable_name, period, max_value, condition = None):
    ...
    if condition is None:
        ...
        condition = input_dataframe_by_entity[entity.key].eval(condition)
        print condition
...
randomly_init_variable(
    ...
    max_value = 50000,
    condition = "household_role == 'first_parent'")  # TODO, condition = simulation.persons.has_role(simulation.famille.DEMANDEUR))  # Randomly set a salaire_net for all persons between 0 and 50000?
randomly_init_variable(tax_benefit_system, input_dataframe_by_entity, 'rent', 2017, max_value = 1000)  # TODO, condition = simulation.persons.has_role(simulation.famille.DEMANDEUR))  # Randomly set a salaire_net for all persons between 0 and 50000?
