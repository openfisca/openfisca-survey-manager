...
def test_asof_simple_annual_parameter():
    ...
    assert parameters.taxes.income_tax_rate(2014) == income_tax_rate_2014, (
        f"{parameters.taxes.income_tax_rate(2014)} != {income_tax_rate_2014}"
    )
    assert parameters.taxes.income_tax_rate(2015) == income_tax_rate_2014, (
        f"{parameters.taxes.income_tax_rate(2015)} != {income_tax_rate_2014}"
    )
...
def test_asof_scale_parameters():
    ...
    assert (
        parameters.taxes.social_security_contribution(2016).thresholds[1]
        == social_security_contribution_2016
    )
    assert (
        parameters.taxes.social_security_contribution(2017).thresholds[1]
        == social_security_contribution_2016
    )
...
def test_inflate_simple_parameter():
    ...
    inflate_parameters(parameters, inflator=0.1, base_year=2016, last_year=2017)
...
def test_inflate_scale_with_changing_number_of_brackets():
    ...
    parameters = tax_benefit_system.parameters
    parameters_asof(
        parameters, instant=periods.instant(2016)
    )  # Remove post 2016 legislation changes
    inflate_parameters(parameters, inflator=0.3, base_year=2016, last_year=2017)
    for threshold_2017, threshold_2016 in zip(
        parameters.taxes.social_security_contribution(2017).thresholds,
        parameters.taxes.social_security_contribution(2016).thresholds,
    ):
        assert threshold_2017 == threshold_2016 * 1.3, (
            f"{threshold_2017} != {threshold_2016 * 1.3}"
        )
...
def test_inflate_start_instant_option():
    ...
    parameters = tax_benefit_system.parameters
    parameters_asof(
        parameters, instant=periods.instant(2022)
    )  # Remove post 2022 legislation changes
    inflate_parameters(
        parameters,
        inflator=0.3,
        base_year=2022,
        last_year=2023,
        start_instant="2023-07-01",
    )
    for threshold_2023_06, threshold_2023_07, threshold_2022 in zip(
        ...
        assert threshold_2023_07 == threshold_2022 * 1.3, (
            f"{threshold_2023_07} != {threshold_2022 * 1.3}"
        )
        assert threshold_2023_06 == threshold_2022, (
            f"{threshold_2023_06} != {threshold_2022}"
        )
