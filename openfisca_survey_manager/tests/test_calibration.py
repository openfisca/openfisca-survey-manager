...
def test_calibration_variable_entity_is_weight_entity():
    ...
    assert_near(
        survey_scenario.compute_aggregate("rent", period=period), target_rent_aggregate
    )
...
def test_calibration_variable_entity_is_not_weight_entity():
    ...
    survey_scenario.calibrate(
        ...
        parameters={
            "method": "raking ratio",
            "id_variable": "household_id",
            "id_variable_link": "household_id_ind",
        },
        other_entity_count=700,
        target_entity_count=300,
    )
    assert_near(
        survey_scenario.compute_aggregate("rent", period=period),
        target_rent_aggregate,
        relative_error_margin=0.1,
    )
    assert_near(
        survey_scenario.compute_aggregate("salary", period=period),
        target_salary_aggregate,
        relative_error_margin=0.1,
    )
    dataframe_by_entity = survey_scenario.create_data_frame_by_entity(
        ["household_weight", "person_weight"], merge=False
    )
    assert_near(
        sum(dataframe_by_entity["household"]["household_weight"]),
        300,
        relative_error_margin=0.1,
    )
    assert_near(
        sum(dataframe_by_entity["person"]["person_weight"]),
        700,
        relative_error_margin=0.1,
    )
...
def test_simulation_calibration_variable_entity_is_weight_entity():
    ...
    assert_near(
        simulation.compute_aggregate("rent", period=period), target_rent_aggregate
    )
    ...
    assert (
        simulation.calculate("household_weight", period).sum()
        == calibration.target_entity_count
    )
...
def test_simulation_calibration_input_from_data():
    ...
    calibration_kwargs = {
        ...
        "target_entity_count": 300,
        "parameters": {"method": "logit", "up": 4, "invlo": 4},
    }
