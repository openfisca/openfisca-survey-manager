...
def test_calibration_variable_entity_is_not_weight_entity():
    ...
    survey_scenario.calibrate(
        ...
        other_entity_count = 70000,
        target_entity_count = 30000,
        )
    ...
    assert_near(survey_scenario.compute_aggregate("person_weight", period = period), 70000, relative_error_margin = 0.1)
    assert_near(survey_scenario.compute_aggregate("household_weight", period = period), 30000, relative_error_margin = 0.1)
