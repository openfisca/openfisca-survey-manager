...
def calmar(
    ...
    Raises:
        SurveyManagerError: Empty margins, invalid variable names, or population mismatch.
    ...
    if hasattr(data, "dummy_is_in_pop") or hasattr(data, "dummy_is_in_pop_smaller_entity"):
        raise SurveyManagerError("dummy_is_in_pop and dummy_is_in_pop_smaller_entity are not valid variable names")
...
def check_calmar(
    ...
    for variable, margin in margins.items():
        if variable != "total_population":
            ...
            log.debug("%s margin=%s relative_diff=%s", variable, margin, rel_diff)
