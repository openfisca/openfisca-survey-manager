...
class Calibration:
    ...
    def __init__(
        ...
        if len(entities) == 0:
            ...
            assert target_entity in [entity.key for entity in simulation.tax_benefit_system.entities]
        elif len(entities) == 2:
            assert "id_variable" in parameters and parameters["id_variable"] is not None, (
                "With two entities involved, an id variable of the largest entity is needed"
            )
            assert "id_variable_link" in parameters and parameters["id_variable_link"] is not None, (
            ...
            assert entity_id_variable_link in entities, (
                "'id_variable' do not correspond to a calibrating variable entity"
            )
            assert entity_id_variable != entity_id_variable_link, (
                "'id_variable_link' must associate a smaller entity to the id of the greater, 'id_variable'"
            )
            ...
            assert numpy.all(numpy.isin(id_link_unique, id_var_unique)) and numpy.all(
                numpy.isin(id_var_unique, id_link_unique)
        ...
        if len(self.entities) == 2:
            for entity in self.entities:
                if entity == self.target_entity:
                    data[entity]["id_variable"] = self.simulation.adaptative_calculate_variable(
                        self.parameters["id_variable"], period=period
                    )
                else:
                    data[entity][self.simulation.weight_variable_by_entity[entity]] = self.initial_weight_by_entity[
                        entity
                    ]
                    data[entity]["id_variable"] = self.simulation.adaptative_calculate_variable(
                        self.parameters["id_variable_link"], period=period
                    )
    ...
    def calibrate(self, inplace: bool = False) -> Optional[np.ndarray]:
        ...
        if margins_by_variable is not None:
            simple_margins_by_variable = {
                variable: margins_by_type["target"] for variable, margins_by_type in margins_by_variable.items()
            }
        else:
            simple_margins_by_variable = {}
    ...
    def set_target_margin(self, variable: str, target: Any) -> None:
        ...
        categorical_variable = (variable_instance.value_type in [bool, Enum] and variable == list_var[0]) or (
            variable_instance.unit in ["years", "months"] and variable == list_var[0]
        )
        for var in list_var:
            expr_categ = var + "[ ]*[<>=!]+"
        ...
        if categorical_variable:
            ...
            filtered_value = (
                value if (filter_by == numpy.array(1.0) or all(filter_by)) else value[filter_by.astype(bool)]
            )
    ...
    def _update_margins(self) -> None:
        ...
        for variable in self.margins_by_variable:
            ...
            if (
                len(self.entities) == 2
                and simulation.tax_benefit_system.variables[list_var[0]].entity.key != self.target_entity
            ):
