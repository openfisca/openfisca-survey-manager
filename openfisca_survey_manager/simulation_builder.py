...
def diagnose_variable_mismatch(used_as_input_variables, input_data_frame):
    ...
    """
    variables_mismatch = (
        set(used_as_input_variables).difference(set(input_data_frame.columns)) if used_as_input_variables else None
    )
...
def init_entity_structure(builder, entity, input_data_frame):
    ...
    id_variables = [builder.id_variable_by_entity_key[_entity.key] for _entity in tax_benefit_system.group_entities]
    role_variables = [
    ...
    if entity.is_person:
        ...
        for group_entity in tax_benefit_system.group_entities:
            ...
            group_population = builder.declare_entity(
                _key, input_data_frame[_id_variable].drop_duplicates().sort_values().values
            )
    ...
    for column_name, column_serie in input_data_frame.items():
        ...
        if entity.is_person:
            ...
            simulation.init_variable_in_entity(
                entity.key, column_name, column_serie[index_by_entity_key[entity.key]], period
            )
