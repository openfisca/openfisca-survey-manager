...
def add_survey_to_collection(
    survey_name=None, survey_collection=None, sas_files=None, stata_files=None, csv_files=None, parquet_files=None
):
...
def check_template_config_files(config_files_directory: str):
    ...
    raw_data_template_ini_path = Path(config_files_directory) / "raw_data_template.ini"
    config_template_ini_path = Path(config_files_directory) / "config_template.ini"

    if Path(config_files_directory).exists():
        config_files_do_not_exist = not (raw_data_ini_path.exists() and config_ini_path.exists())
        templates_config_files_do_not_exist = not (
            raw_data_template_ini_path.exists() and config_template_ini_path.exists()
        )
...
def main():
    ...
    parser.add_argument(
        "-p", "--path", help=f"path to the config files directory (default = {default_config_files_directory})"
    )
