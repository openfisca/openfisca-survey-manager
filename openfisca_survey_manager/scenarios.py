...
from openfisca_core.entities import GroupEntity  # type: ignore
from openfisca_core.indexed_enums import Enum  # type: ignore
from openfisca_core.periods import Period, MONTH, YEAR, ETERNITY  # type: ignore
...
from openfisca_core.tools.simulation_dumper import dump_simulation, restore_simulation  # type: ignore
...
class AbstractSurveyScenario(object):
    ...
    id_variable_by_entity_key: Optional[Dict[str, str]] = None
    ...
    role_variable_by_entity_key: Optional[Dict[str, str]] = None
    simulation = None
    ...
    def compute_aggregate(
            ...
            use_baseline = False,
            difference = False,
            missing_variable_default_value = numpy.nan
            ):
        ...
        if filter_by:
            assert filter_by in self.tax_benefit_system.variables, \
                f"{filter_by} is not a variables of the tax benefit system"
        ...
        if variable in simulation.tax_benefit_system.variables:
            value = self.calculate_variable(
                variable = variable,
                period = period,
                use_baseline = use_baseline,
                )
        else:
            log.debug(f"Variable {variable} not found. Assiging {missing_variable_default_value}")
            return missing_variable_default_value

        weight = (
            self.calculate_variable(
                variable = entity_weight,
                period = period,
                use_baseline = use_baseline,
                ).astype(float)
        ...
        filter_dummy = self.calculate_variable(
            variable = filter_by,
            period = period,
            ) if filter_by else 1.0
    ...
    def compute_pivot_table(
        ...
        if self.weight_column_name_by_entity is not None:
            ...
            log.debug(f"There is no weight variable for entity {entity_key}")
            weight = None
        ...
        for variable in variables:
            assert tax_benefit_system.variables[variable].entity.key == entity_key, \
                "The variable {variable} is not present or does not belong to entity {entity_key}"
        ...
        if values:
            ...
            if len(list(data_frame_by_value.keys())) > 1:
                if concat_axis is None:
                    ...
                    return pandas.concat(data_frame_by_value.values(), axis = concat_axis)
            else:
    ...
    def calculate_variable(self, variable = None, period = None, use_baseline = False):
        ...
        assert variable in tax_benefit_system.variables, f"{variable} is not a valid variable"
        ...
        assert values is not None, 'Unspecified calculation period for variable {}'.format(
            variable)
    ...
    def create_data_frame_by_entity(
        ...
        if use_baseline and self.baseline_simulation:
            simulation = self.baseline_simulation
        else:
            simulation = self.simulation

        if use_baseline and self.baseline_tax_benefit_system:
        ...
        if missing_variables:
            log.info(f"These variables aren't part of the tax-benefit system: {missing_variables}")
        ...
        assert len(columns_to_fetch) >= 1, f"None of the requested variables {variables} are in the tax-benefit-system {list(tax_benefit_system.variables.keys())}"
        assert simulation is not None
        ...
        if index:
            ...
            if person_data_frame is None:
                person_data_frame = pandas.DataFrame()

            for entity in non_person_entities:
                person_data_frame[f"{entity.key}_id"] = simulation.entities[entity.key].members_entity_id
                person_data_frame[f"{entity.key}_role"] = simulation.entities[entity.key].members_legacy_role
                person_data_frame[f"{entity.key}_entity.key"] = simulation.entities[entity.key].members_position
    ...
    def init_all_entities(self, input_data_frame, simulation, period = None, entity = None):
        assert period is not None
        if entity:
            log.info(f"Initialasing simulation using input_data_frame for entity {entity} for period {period}")
        else:
        ...
        if period.unit == YEAR:  # 1. year
            ...
            log.info(f"Invalid period {period}")
            raise
    ...
    def filter_input_variables(self, input_data_frame = None, simulation = None):
        ...
        if used_columns:
            log.debug(f"These columns are not dropped because present in used_as_input_variables:\n {sorted(used_columns)}")

        if dropped_columns:
            log.debug(f"These columns in survey are set to be calculated, we drop them from the input table:\n {sorted(dropped_columns)}")

        log.info(f"Keeping the following variables in the input_data_frame:\n {sorted(list(input_data_frame.columns))}")
        ...
        for use_baseline in [False, True]:
            ...
            for variable_name in set(inflator_by_variable.keys()).union(set(target_by_variable.keys())):
                ...
                if variable_name in target_by_variable:
                    ...
                    log.info(f"Using {inflator} as inflator for {variable_name} to reach the target {target_by_variable[variable_name]}")
                    ...
                    log.info('Using inflator {} for {}.  The target is thus {}'.format(
                        ...
                        variable_name,
                        inflator_by_variable[variable_name] * self.compute_aggregate(
                            variable = variable_name,
                            use_baseline = use_baseline,
                            period = period,
                            )
                ...
                simulation.delete_arrays('variable_name', period = period)

                # insert inflated array
                simulation.set_input(variable_name, period, inflator * array)
    ...
    def init_from_data(
        ...
        self._id_variable_by_entity_key()
        self._role_variable_by_entity_key()
        ...
        # Note that I can pass a :class:`pandas.DataFrame` directly, if I don't want to rebuild the data.
        self.new_simulation(
            debug = debug,
            data = data,
            trace = trace,
    ...
    def init_entity_structure(
        ...
        id_variables: List[str] = [
            self._id_variable_by_entity_key()[entity.key]
            for entity in tax_benefit_system.group_entities
            ]

        role_variables: List[str] = [
            self._role_variable_by_entity_key()[entity.key]
            for entity in tax_benefit_system.group_entities
            ]
        ...
        if entity.is_person:
            ...
            for collective_entity in tax_benefit_system.group_entities:
                ...
                _id_variable = self._id_variable_by_entity_key()[_key]
                _role_variable = self._role_variable_by_entity_key()[_key]
                ...
                for (legacy_role, flattened_role) in enumerate(collective_entity.flattened_roles):
                    if legacy_role < len(collective_entity.flattened_roles):
                        collective_entity.members_role = numpy.where(
                        ...
                        collective_entity.members_role = numpy.where(
                            collective_entity.members_legacy_role >= len(
                                collective_entity.flattened_roles),
    ...
    def init_simulation_with_data_frame(self, input_data_frame = None, period = None, simulation = None, entity = None):
        ...
        if variables_mismatch:
            log.info(f"The following variables used as input variables are not present in the input data frame: \n {sorted(variables_mismatch)}")
        ...
        role_variables = [
            role_variable_by_entity_key[entity.key]
            for entity in simulation.entities.values()
            if not entity.is_person
            ]
        ...
        input_data_frame = self.filter_input_variables(
            input_data_frame = input_data_frame,
            simulation = simulation,
            )
        ...
        for key, entity in simulation.entities.items():
            if entity.is_person:
                ...
                entity.count = entity.step_size = (input_data_frame[role_variable_by_entity_key[key]] == 0).sum()
        ...
        for column_name, column_serie in input_data_frame.iteritems():
            ...
            if entity.is_person:
                ...
                init_variable_in_entity(
                    entity,
                    column_name,
                    column_serie[index_by_entity_key[entity.key]],
                    period,
                    )
    ...
    def init_simulation(self, tax_benefit_system: TaxBenefitSystem, period: Period, data: dict) -> None:
        builder: SimulationBuilder = SimulationBuilder()
        builder.create_entities(tax_benefit_system)

        data_year: int = data.get("data_year", self.year)
        ...
        assert len(source_types) >= 1, f"There should be one data source type included in {default_source_types}"
        ...
        if source_type == 'input_data_table':
            ...
            if input_data_survey_prefix is not None:
                ...
                openfisca_survey = openfisca_survey_collection.get_survey(f"{input_data_survey_prefix}_{data_year}")
            ...
            self.init_all_entities(input_data_frame, simulation, period)
    ...
    def load_table(
        ...
        if survey is not None:
            ...
            survey = f"{self.input_data_survey_prefix}_{self.year}"
    ...
    def restore_simulations(self, directory, **kwargs):
        ...
        use_sub_directories = os.path.exists(
            os.path.join(directory, 'baseline'))
    ...
    def summarize_variable(self, variable = None, use_baseline = False, weighted = False, force_compute = False):
        ...
        if not infos:
            if force_compute:
                ...
                print("{variable} is not computed yet. Use keyword argument force_compute = True")
                return
        ...
        if holder is not None:
            if holder.variable.definition_period == ETERNITY:
                ...
                print("permanent: mean = {}, min = {}, max = {}, median = {}, default = {:.1%}".format(
                    array.mean() if not weighted else numpy.average(array, weights = weights),
                    array.min(),
                    array.max(),
                    numpy.median(array),
                    (
                ...
                for period in sorted(holder.get_known_periods()):
                    ...
                    if value_type == Enum:
                        ...
                        categories_type = pandas.api.types.CategoricalDtype(
                            categories = possible_values._member_names_,
                            ordered = True,
                            )

                        df = pandas.DataFrame({variable: array}).replace(categories_by_index).astype(categories_type)
                    ...
                    print("{}: mean = {}, min = {}, max = {}, mass = {:.2e}, default = {:.1%}, median = {}".format(
                        ...
                        array.min(),
                        array.max(),
                        array.sum() if not weighted else numpy.sum(array * weights),
                        (
                            (array == default_value).sum() / len(array)
                            if not weighted
                            else ((array == default_value) * weights).sum() / weights.sum()
    ...
    def _id_variable_by_entity_key(self) -> Dict[str, str]:
        '''Identify and set the good ids for the different entities'''
        if self.id_variable_by_entity_key:
        ...
        self.id_variable_by_entity_key = dict(
            (entity.key, entity.key + '_id')
            for entity in self.tax_benefit_system.entities
            )

        log.debug(f"Use default id_variable names:\n  {self.id_variable_by_entity_key}")

        return self.id_variable_by_entity_key

    def _role_variable_by_entity_key(self) -> Dict[str, str]:
        '''Identify and set the good roles for the different entities'''
        if self.role_variable_by_entity_key:
        ...
        self.role_variable_by_entity_key = dict(
            (entity.key, entity.key + '_legacy_role')
            for entity in self.tax_benefit_system.entities
            )
    ...
    def _set_used_as_input_variables_by_entity(self) -> Dict[str, List[str]]:
        ...
        if self.used_as_input_variables_by_entity:
...
def assert_variables_in_same_entity(survey_scenario, variables):
    ...
    for variable_name in variables:
        ...
        assert variable.entity == entity, f"{variables} are not from the same entity: {variable_name} doesn't belong to {entity.key}"
...
def get_entity(survey_scenario, variable):
    ...
    assert variable_, f"Variable {variable} is not part of the tax-benefit-system"
...
def init_variable_in_entity(entity, variable_name, series, period):
    ...
    if series.values.dtype != variable.dtype:
        log.debug(f"Converting {variable_name} from dtype {series.values.dtype} to {variable.dtype}")

    if numpy.issubdtype(series.values.dtype, numpy.floating):
    ...
    np_array = numpy.array(array, dtype = variable.dtype)
    ...
    if variable.definition_period == YEAR and period.unit == MONTH:
        ...
        if simulation.get_array(variable_name, period.this_year) is not None:
            array_sum = simulation.get_array(
                variable_name,
                period.this_year,
                ) + np_array
