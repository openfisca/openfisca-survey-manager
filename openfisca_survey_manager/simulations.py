...
def inflate(
    ...
    for variable_name in set(inflator_by_variable.keys()).union(
        ...
        if variable_name in target_by_variable:
            ...
            log.info(
                "Using {} as inflator for {} to reach the target {} ".format(
            ...
            log.info(
                "Using inflator {} for {}.  The target is thus {}".format(
...
def summarize_variable(
    ...
    if variable_instance.is_neutralized:
        ...
        print(
            "{}: neutralized variable ({}, default = {})".format(
                variable, str(np.dtype(value_type)), default_value
            )
    ...
    if not infos:
        if force_compute:
            ...
            print(
                "{} is not computed yet. Use keyword argument force_compute = True".format(
                    variable
                )
            )  # noqa analysis:ignore
            return
        ...
        if holder.variable.definition_period == ETERNITY:
            ...
            for period in sorted(simulation.get_known_periods(variable)):
                ...
                if value_type == Enum:
                    ...
                    total = groupby.sum()
                    expr = [
                        " {} = {:.2e} ({:.1%})".format(index, row, row / total)
